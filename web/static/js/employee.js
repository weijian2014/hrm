...
function genderFormatter(value, row) {
   var icon = value == "男" ? 'fa-male' : 'fa-female'
...
//       idField: 'ids',                      //表明哪个是字段是标识字段
//       uniqueId: "ids",                     //表明每一行的唯一标识字段，一般为主键列
//       showToggle: true,                    //是否显示详细视图和列表视图的切换按钮
//       cardView: false,                    //是否显示详细视图  设置为true以显示卡片视图表，例如mobile视图（卡片视图）
//       detailView: false,                  //设置为true以显示detail 视图表（细节视图）
//       locale: 'zh-CN',
//       classes: 'table table-hover ',  //表格样式。可以使用'table'，'table-bordered'，'table-hover'，'table-striped'，'table-dark'，'table-sm'和'table-borderless'。默认情况下，表格是有界的。
...
//       singleSelect: true,// 默认false，设为true则允许复选框仅选择一行(不能多选了？)
...
function initTable2() {
   $table.bootstrapTable('destroy').bootstrapTable({
      ...
      showExtendedPagination: true,       // 分页显示的增强, 比如搜索时可以显示 从 xx 总记录中过滤
      sortable: true,                     //是否启用排序  列中也有此变量
      sortName: '',                        //定义要排序的列, 没定义默认都不排列，同sortOrder结合使用，sortOrder没写的话列默认递增（asc）
      sortOrder: "asc",                   //定义列排序顺序，只能是'asc'或'desc'。
      sortStable: false,                   //如果你把此属性设为了true）我们将为此行添加'_position'属性 （别看错了，是sortStable，sortable在下面）设为true，则和sort部分一样，区别是：在排序过程中，如果存在相等的元素，则原来的顺序不会改变

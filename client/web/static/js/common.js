...
function genTable(tData, keyWord, isAll) {
  if (1 != tData.response) {
    if ("" != keyWord) {
      $(".resultTableDiv").html(
        `<h2>无[` + keyWord + `]对应的搜索结果，请尝试其它关键字</h2>`
      );
      return;
    } else {
  ...
  if ("" == keyWord) {
    ...
    tableStr +=
      `<p style="height: 25px; font-size: 14px; padding-top: 8px;">
                <span style="color:#FFF; padding-top: 8px;">共<span id="rowCount" style="color:#0BE093;">` +
      totalCount +
      `</span>行</span>
                <span style="color:#FFF;">分<span id="rowCount" style="color:#0BE093;">` +
      pagingTotal +
      `</span>页</span>
                <a style="color:#FFF;" href="#" onclick="doPaging(` +
      isAll +
      `, 1)">首页</a>&nbsp;&nbsp;`;
    ...
    for (var i = 1; i <= pagingTotal; i++) {
      tableStr +=
        `<a id="pageIndexId` +
        i +
        `" style="color:#FFF;" href="#" onclick="doPaging(` +
        isAll +
        `, ` +
        i +
        `)">` +
  ...
  if (isAll) {
    ...
    for (var i = 0; i < rowCount; i++) {
      tableStr +=
        ...
        tData.msg.row_infos[i].id +
        `</td>
                <td onclick="tdClick(this)">` +
        tData.msg.row_infos[i].vehicle_name +
        `</td>
                <td onclick="tdClick(this)">` +
        tData.msg.row_infos[i].parts_name +
        `</td>
                <td onclick="tdClick(this)">` +
        tData.msg.row_infos[i].parts_no +
        `</td>
                <td onclick="tdClick(this)">` +
        tData.msg.row_infos[i].parts_count +
        `</td>
                <td onclick="tdClick(this)">` +
        tData.msg.row_infos[i].parts_price +
        `</td>
                <td onclick="tdClick(this)">` +
        tData.msg.row_infos[i].parts_selling_price +
        `</td>
